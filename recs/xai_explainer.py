...
def get_lightfm_feature_importance(user_id, movie_id, model, items):
    """Extract approximate feature importance from LightFM embeddings."""
    try:
        ...
        logger.exception("Feature importance extraction failed: %s", e)
        return None


def compute_shap_like_values(user_id, movie_id, model, items):
    """Compute SHAP-like weights for recommendation factors."""
    try:
        ...
        logger.exception("SHAP-like computation failed: %s", e)
        return None


def get_lime_explanation(user_id, movie_id):
    """Generate local explanation factors for one movie recommendation."""
    try:
        ...
        logger.exception("LIME-style explanation failed: %s", e)
        return []


def get_comprehensive_xai_explanation(user_id, movie_id, model=None, items=None):
    """Combine SHAP-like, LIME-like, and LightFM signals into one payload."""
    explanation = {
        'shap_values': None,
